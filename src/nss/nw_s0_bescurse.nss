...
// End of Spell Cast Hook
    ...
    effect eAB  = EffectAttackDecrease(4);
    effect eSaves = EffectSavingThrowDecrease(SAVING_THROW_ALL, 4);
    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        SignalEvent(oTarget, EventSpellCastAt(oTarget, SPELL_BESTOW_CURSE));
         ...
         if (!MyResistSpell(OBJECT_SELF, oTarget))
         {
            ...
            if (GetLastSpellCastClass() == CLASS_TYPE_CLERIC || CLASS_TYPE_WIZARD ||CLASS_TYPE_SORCERER)
            {
                ApplyEffectToObject(DURATION_TYPE_PERMANENT, eAB, oTarget);
                ApplyEffectToObject(DURATION_TYPE_PERMANENT, eSaves, oTarget);
            }
        }
    }
}
