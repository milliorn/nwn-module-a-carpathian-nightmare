...
void main()
{
    ...
    {
         if(GetIsFriend(oObject))
         {
            // Apply effects to caster
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eAttack, oPC, TurnsToSeconds(nDur));
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eDamage, oPC, TurnsToSeconds(nDur));
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eSave, oPC, TurnsToSeconds(nDur));
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eSkill, oPC, TurnsToSeconds(nDur));
         ...
         oObject = GetNextObjectInArea(oPC);
    }
/*
    // Apply effects to caster
    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eAttack, oPC, TurnsToSeconds(nDur));
