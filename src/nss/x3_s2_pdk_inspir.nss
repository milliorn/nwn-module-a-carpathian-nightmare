...
void main()
{
    ...
    int nClass = GetLevelByClass(CLASS_TYPE_PURPLE_DRAGON_KNIGHT);

    effect eAttack = EffectAttackIncrease(nClass);// Attack effect increased +1
    effect eDamage = EffectDamageIncrease(nClass, DAMAGE_TYPE_BLUDGEONING);// Increased damage effect
    effect eLink = EffectLinkEffects(eAttack, eDamage);// link effects

    effect eSave = EffectSavingThrowIncrease(SAVING_THROW_WILL, nClass);// Saving throw increase
