...
// End of Spell Cast Hook
    ...
    //Custom Code to make the spell 3.5/5.0 edition compliant

    // Create the effect to apply
    effect eImmune = EffectImmunity(IMMUNITY_TYPE_POISON);
    eVis = EffectVisualEffect(VFX_IMP_GOOD_HELP);

    // Apply the visual effect to the target
    ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oTarget);
    // Apply the effect to the object
    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eImmune, oTarget, TurnsToSeconds(10 + nCasterLevel));
}
