...
void main()
{
    ...
    if (GetPlotFlag(OBJECT_SELF) && GetResRef(OBJECT_SELF) != "gauntletmaster")
    {
        FloatingTextStringOnCreature("<cþf >WARNING!  Do not go hostile"
        + "<cþf > on non-hostile NPC's.  Now, escape from jail!", oAttacker);
        DelayCommand(2.0, AssignCommand(oAttacker, JumpToLocation(GetLocation(GetWaypointByTag("WP_JAIL")))));
        return;
    }
