...
void main()
{
    ...
    int iClass = GetLevelByClass(CLASS_TYPE_PURPLE_DRAGON_KNIGHT, oPDK);
    int iAbility = GetAbilityModifier(ABILITY_CHARISMA, oPDK);
    int nCount = iClass + iAbility;

    int nHP = d10(2) + nCount;
    ...
    while (GetIsObjectValid(oTarget) && nTargetsLeft > 0)
    {
        if(oTarget == OBJECT_SELF)
        {
            ...
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eHP, oTarget, TurnsToSeconds(nCount));
