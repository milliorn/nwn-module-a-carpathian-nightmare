...
void main()
{
    ...
    int nDC = GetLevelByClass(CLASS_TYPE_ARCANE_ARCHER, OBJECT_SELF);
    ...
    if (GetIsObjectValid(oTarget) == TRUE)
    {
        ...
        if (nTouch > 0)
        {
            int nDamage = ArcaneArcherDamageDoneByBow(nTouch ==2) + nBonus;
            if (nDamage > 0)
            {
                ...
                if (MySavingThrow(SAVING_THROW_FORT, oTarget, 10 + nDC) == 0)
                {
