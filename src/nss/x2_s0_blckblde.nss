...
void spellsCreateItemForSummoned()
{
    ...
    int nStat;
    int nCastingLevel = GetCasterLevel(OBJECT_SELF);
...
//    SetPlotFlag (oSummon,TRUE);
    ...
    if (nCastingLevel >= 35)
    {
    AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_NEGATIVE, IP_CONST_DAMAGEBONUS_2d8), oWeapon);
    AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_MAGICAL, IP_CONST_DAMAGEBONUS_2d8), oWeapon);
    }
    else if (nCastingLevel >= 30)
    {
    AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_MAGICAL, IP_CONST_DAMAGEBONUS_2d8), oWeapon);
    }
    else if (nCastingLevel >= 25)
    {
    AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_MAGICAL, IP_CONST_DAMAGEBONUS_1d8), oWeapon);
    }
    if (GetHasFeat(FEAT_EPIC_SPELL_FOCUS_CONJURATION))
    {
    AddItemProperty(DURATION_TYPE_PERMANENT, ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_ELECTRICAL, IP_CONST_DAMAGEBONUS_2d12), oWeapon);
    }
}
...
// End of Spell Cast Hook
    ...
    effect eSummon = EffectSummonCreature("su_blackblade");
